...
**Step 4 — collector.** `wata.collect` polls the core+rotation sample every 20 minutes across the 05:54–22:57 span (~51 calls/day, ~1,530/month — right at the ceiling; narrow the window or widen the interval once Phase 4's O-D `plan()` calls need their own budget). A persisted `poll_index` (`data/processed/poll_index.json`) advances the rotation across runs, since each GitHub Actions run is a fresh checkout. Appends raw responses as gzipped NDJSON, one file per day, committed back by the workflow. `wata.transit_api`'s `CallBudget` refuses any call that would exceed the monthly cap rather than risking the key getting throttled server-side.
...
- **Reliability is prediction-based, not observed, and only ~50% of trips get even that.** Calibration showed `is_real_time` populates only in roughly the last 10 minutes before departure; at 20-minute polling, about half of trips will have no real-time-flagged reading at all, only a schedule guess. Label both caveats on the dashboard, not just a footnote.
